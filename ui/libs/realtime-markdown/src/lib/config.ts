import { calcHeadingSize, calcIndent } from './utils/elements.utils';
...
export const STATIC_PARSERS: ParserType[] = [
  ...
  },
  {
    ...
    toHtml: (innerHtml: string) => {
      ...
      const emmet = DYNAMIC_ELEMENTS_CONFIG['a'].initialEmmet({
  ...
  },
  {
    id: 'a',
    regex:
      /https?:\/\/(www\.)?[-a-zA-Z\d@:%._+~#=]{1,256}\.[a-zA-Z\d]{1,6}\b([-a-zA-Z\d()@:%_+.~#?&\\/=]*)/gm,
    toHtml: (innerHtml: string) => {
      innerHtml = innerHtml.replace(/(( |^|>)|( |$|<))/g, '');
      const emmet = DYNAMIC_ELEMENTS_CONFIG['a'].initialEmmet({
        innerHtml: innerHtml,
...
// .et-* === .element-type-*
export const DYNAMIC_ELEMENTS_CONFIG: {
  [tagsEmmet: string]: ElementRepresentationConfig;
} = {
  h1: {
